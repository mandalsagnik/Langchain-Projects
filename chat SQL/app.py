...
st.set_page_config(page_title="Lanchain: Chat with SQL DB")
st.title("LangChain: Chat with SQL DB")

LOCALDB="USE_LOCALDB"
MYSQL="USE_MYSQL"

radio_opt=["Use SQLLTE 3 Database-Student.db","connect to you SQL Database"]

selected_opt=st.sidebar.radio(label="Choose the DB which want to chat",options=radio_opt)
...
if not db_url:
    st.title("Please Enter The Database Information and Url")

if not api_key:
    st.title("Please Add the Groq API Key")
...
def configure_db(db_url, mysql_host=None, mysql_user=None, mysql_password=None, mysql_db=None):
    if db_url == LOCALDB:
        ...
        if not (mysql_host and mysql_user and mysql_password and mysql_db):
            ...
            st.stop()
        safe_password = quote_plus(mysql_password)   # 👈 encodes @, :, / correctly
        return SQLDatabase(create_engine(
            f"mysql+mysqlconnector://{mysql_user}:{safe_password}@{mysql_host}/{mysql_db}"
        ))
